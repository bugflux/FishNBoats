...
public class PortoAveiro {
	...
	public static void main(String[] args) {
		...
		/*
		 * TLogFlusher logFlusher = null; if (logFile == null) { logFlusher =
		 * new TLogFlusher(System.out); } else { try { logFlusher = new
		 * TLogFlusher(new FileOutputStream(logFile)); } catch
		 * (FileNotFoundException e) {
		 * System.out.println("Error opening file for logging");
		 * System.exit(-1); } } logFlusher.start();
		 */
		// Ocean
		Point wharf = new Point(0, 0);
		Point reproducingZone = new Point(width - 1, height - 1);
		MOcean oceano = new MOcean(height, width, maxShoalPerSquare, maxBoatsPerSquare, wharf, reproducingZone, clogger);
		...
		for (int r = 0; r < nshoals; r++) {
			sShoals[r] = new ShoalStats(new ShoalId(r), INTERNAL_STATE_SCHOOL.spawning, new Point(reproducingZone),
			...
			tShoals[r] = new TShoal((ShoalStats) sShoals[r].clone(), shoalPeriod, seasonMoves, nCampaign, cShoals[r],
					oceanClient, cDirOpers, growing_factor, eco_system, catchPercentage);

		}

		for (int r = 0; r < ncompanies; r++) {

			sDirOpers[r] = new DirOperStats(INTERNAL_STATE_DIROPER.starting_a_campaign, new DirOperId(r));
			...
			tDirOpers[r] = new TDirOper(oceanClient, cDirOpers[r], cBoats[r], cShoals,
					(DirOperStats) sDirOpers[r].clone());

			for (int s = 0; s < nboats; s++) {
				sBoats[r][s] = new BoatStats(new BoatId(r, s), INTERNAL_STATE_BOAT.at_the_wharf, new Point(wharf),
						boatCapacity);
				...
				tBoats[r][s] = new TBoat((BoatStats) sBoats[r][s].clone(), boatPeriod, cDirOpers[r], oceanClient,
						cBoats[r][s]);
