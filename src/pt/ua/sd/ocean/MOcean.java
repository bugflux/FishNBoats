...
public class MOcean implements IOceanBoat, IOceanShoal, IOceanDirOper {
	...
	public MOcean(int height, int width, int maxShoalPerSquare,
			int maxBoatsPerSquare, Point wharf, Point reproducingZone) {
		assert (height > 0);
		assert (width > 0);
		...
		map[reproducingZone.y][reproducingZone.x] = new Cell(0,
				Integer.MAX_VALUE);
	...
	/**
	 * @see #addBoat(BoatStats, Point)
	 */
	public void addBoat(BoatStats boat, int x, int y) {
		synchronized (this) {
			addBoat(boat, new Point(x, y));
		}
	}
		...
		synchronized (this) {
			...
			if (c.equals(p)) {
				...
				Random rand = new Random(new Date().getTime()
						* Thread.currentThread().getId() * id.hashCode());
	...
	public IShoalBoat companionDetected(BoatId id, BoatId helper) {
		...
		log.push("Scan companion", id.toString(), logCompanion + ", "
				+ logShoal, logTick);
	...
	public List<Point> getRadar(BoatId id) {
		...
		if (points.isEmpty()) {
			...
			for (int r = 1; r < points.size(); r++) {
				logMessage.append(", (" + points.get(r).y + ","
						+ points.get(r).x + ")");
			}
		}
